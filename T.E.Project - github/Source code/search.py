...
def search():
    ...
    db = mysql.connector.connect(host="localhost", user="root", password="", database="admission3")
    mycursor = db.cursor(buffered = True)
    if option == "Name":
        mycursor.execute("SELECT * FROM form WHERE fname = '" + str(search_input) + "'")
    elif option == "Aadhar no":
        mycursor.execute("SELECT * FROM form WHERE Aadharno = '" + str(search_input) + "'")
    else:
        return
...
def draw_result():
    ...
    dept=Label(result, text = "Category : ", font=("Minion Pro Regular",20), bd = 0, bg = "#777", fg = "#fff")
    ...
    desg=Label(result, text = "Date of Birth : ", font=("Minion Pro Regular", 20), bd = 0, bg = "#777", fg = "#fff")
...
#heading label
nsec=Label(header, text="SVPM's College of Engineering Malegaon B.K",font=("Helvetica", 28,"bold"), bg = "Orange",fg="#eae2b7")
nsec.place(x=0, y=10, width=900)

#Profile frame
frame2=Frame(root, bg="#fbb1bd")
frame2.place(x=0,y=75,width=900,height=50)
welcome_text = Label(frame2, text = "Search Student (by Name OR Aadhar No)", font=("Minion Pro Regular", 16), bg="#fbb1bd")
...
dropdown['values']=("--search by--", "Name", "Aadhar No")
dropdown.current(0)
